...
import 'rolling_message_ui/message_input.dart';

class RollingOverListTile extends StatelessWidget {
  final RollingOverChatModel rollingOverChatModel;
  final DashboardController dashboardController = Get.put(DashboardController());
  RollingOverListTile({super.key, required this.rollingOverChatModel});
  ...
  Widget build(BuildContext context) {
    final safeName = cleanName(rollingOverChatModel.profileName);
    final Color brandColor = _brandColor;

    return InkWell(
      onTap: () {
        Profile rollingConvertedModel = Profile(
            profileWaId: rollingOverChatModel.profileWaId,
            profileWaKey: rollingOverChatModel.profileWaKey,
      ...
      child: Column(
        children: [
          ListTile(
            tileColor: rollingOverChatModel.getPendingMsgCount > 0
            ...
            trailing: Column(
              ...
              children: [
                /// ✅ Date text — brand color if unread
                Text(
                  formatChatDate(rollingOverChatModel.updatedTime),
                  style: TextStyle(
                    fontSize: 12,
                    color: rollingOverChatModel.getPendingMsgCount > 0
                        ? brandColor
                        : Colors.grey,
                    fontWeight: FontWeight.w500,
                  ),
                ...
                if (rollingOverChatModel.getPendingMsgCount > 0)
                  CircleAvatar(
                    radius: 10,
                    backgroundColor: brandColor,
                    child: Center(
                      child: Text(
                        rollingOverChatModel.getPendingMsgCount.toString(),
...
//             getPendingMsgCount:rollingOverChatModel.getPendingMsgCount,
//             updatedTime: rollingOverChatModel.updatedTime,
//             hasVoiceCallingPermission: rollingOverChatModel.hasVoiceCallingPermission);
...
//         children: [
//           ListTile(
//             tileColor: rollingOverChatModel.getPendingMsgCount > 0 ? AppTheme.unreadMessagesColor.withOpacity(0.1) : null,
